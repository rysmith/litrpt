...
rpt_name=`date +%s`_SQLreport.sql
rpt_num=`date +%s`
...
prod_size=`du -h . | tail -n 1 | sed 's/[ \t]//g' | cut -d '.' -f 1`
...
echo ""
echo "Great! Thanks for the input.  Generating SQL input statements and error reports..." 
echo ""
...
touch $ext_tmp
touch $sorted_ext_tmp
touch $counted_ext_tmp
...
echo "************************************************"
cat ~/rsmith/litrpt/$rpt_name

echo ""
echo "*******************************************************"
echo "*ADDITIONAL MESSAGES REGARDING ${party^^}'S PRODUCTION*"
echo "*******************************************************"
